...
function listCatalogEntries() {
	return PLUGIN_CATALOG.map(decorateEntry);
}
function getCatalogEntry(slug) {
	const entry = PLUGIN_CATALOG.find((candidate) => candidate.slug === slug);
	return entry ? decorateEntry(entry) : void 0;
}
function getCatalogAvailabilityRules() {
	return REGISTRY_CATALOG_AVAILABILITY;
}
const LOCAL_ICON_PATHS = Object.fromEntries(Object.entries(REGISTRY_CATALOG_LOCAL_ICONS).map(([slug, icon]) => [slug, icon.kind === "single" ? icon.path : icon.light]));
const registryPopularity = REGISTRY_CATALOG_POPULARITY;
const PLUGIN_CATALOG = REGISTRY_CATALOG_ENTRIES.map((entry) => Schema.decodeUnknownSync(PluginRegistryEntry)(entry));
function decorateEntry(entry) {
	const icon_url = entry.icon_url ?? LOCAL_ICON_PATHS[entry.slug];
	const popularity = entry.popularity ?? registryPopularity.entries[entry.slug] ?? registryPopularity.default;
...
//#endregion
export { getCatalogAvailabilityRules, getCatalogEntry, listCatalogEntries };
