...
const harborOpenApiDocument = createHarborOpenApiDocument();
const normalizeForJson = (value) => {
	if (Array.isArray(value)) return value.map(normalizeForJson);
	if (!value || typeof value !== "object") return value;
	const input = value;
	const output = {};
	for (const key of Object.keys(input).sort()) output[key] = normalizeForJson(input[key]);
	return output;
};
...
//#endregion
export { HARBOR_OPENAPI_ALIAS_PATH, HARBOR_OPENAPI_PATH, HARBOR_OPENAPI_VERSION, createHarborOpenApiDocument, harborOpenApiComponents, harborOpenApiDocument, harborProtocolOperations, stableJsonStringify };
