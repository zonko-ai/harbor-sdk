...
const PolicyActorSchema = Schema.Struct({
...
const PolicyRuleRecord = Schema.Struct({
	id: Schema.String,
	workspace_id: Schema.String,
	scope: PolicyScope,
...
const POLICY_TOKEN_PATTERN = /^[a-z0-9][a-z0-9_-]*$/;
function policyTarget(namespace, name) {
	return [namespace, name].map(normalizePolicyToken).join(".");
}
function validatePolicyPattern(pattern) {
	return parsePattern(pattern) !== null;
}
function normalizePolicyIdentifier(value) {
	return normalizePolicyToken(value);
}
function resolvePolicyRule(input) {
	const target = input.targetPatterns.map(parseTarget).filter((entry) => Boolean(entry));
	if (target.length === 0) return {
		action: input.defaultAction ?? "allow",
		reason: "invalid_target",
		operation: input.operation,
		code: "policy.invalid_target",
		message: "Policy target is malformed."
	};
	const sortedPolicies = [...input.rules ?? []].filter((policy) => policy.enabled !== false).filter((policy) => policyOperationApplies(policy, input.operation)).filter((policy) => policyAppliesToActor(policy, input.workspaceId, input.actor)).sort(comparePolicyOrder);
	for (const policy of sortedPolicies) {
		const pattern = parsePattern(policy.pattern);
		if (!pattern) continue;
		if (!target.some((candidate) => matchesPattern(pattern, candidate))) continue;
		return {
			...
			...policy.reasonText ? { message: policy.reasonText } : {}
		};
	}
	return {
		action: input.defaultAction ?? "allow",
...
function resolveToolExecutionPolicy(input) {
	const operation = input.operation ?? "tool.invoke";
	const sourceKind = nonEmpty(input.target.sourceKind);
	const supportedSourceKinds = input.supportedSourceKinds ?? [
		"mcp",
		"api",
		"cli",
		"composio"
	];
	if (sourceKind && !supportedSourceKinds.includes(sourceKind)) return {
		action: "deny",
		reason: "source_kind_unsupported",
		operation,
		code: "policy.source_kind_unsupported",
		message: "Tool source kind is not supported for this execution surface."
	};
	if (sourceKind === "api" && input.allowApiSources === false) return {
		action: "deny",
		reason: "feature_disabled",
		operation,
		code: "policy.api_sources_disabled",
		message: "API source execution is disabled for this environment."
	};
	if (input.target.sourceStatus && input.target.sourceStatus !== "ready") return {
		action: "deny",
		reason: "source_not_runnable",
		operation,
		code: "policy.source_not_ready",
		message: "Tool source is not ready."
	};
	if (input.target.sourceToolCount !== void 0 && input.target.sourceToolCount !== null && input.target.sourceToolCount <= 0) return {
		action: "deny",
		reason: "source_not_runnable",
		operation,
		code: "policy.source_has_no_tools",
		message: "Tool source has no callable tools."
	};
	const target = executionTargetAliases(input.target);
	return resolvePolicyRule({
		workspaceId: input.workspaceId,
		operation,
		actor: input.actor,
		targetPatterns: target.map((parts) => parts.join(".")),
		rules: input.rules,
		defaultAction: input.defaultAction
	});
}
function executionTargetAliases(target) {
	const aliases = [];
	const parsedToolId = parseTarget(target.toolId);
	if (parsedToolId) aliases.push(parsedToolId);
	const namespace = nonEmpty(target.sourceNamespace);
	const toolName = nonEmpty(target.toolName);
	if (namespace && toolName) {
		const parsed = parseTarget(policyTarget(namespace, toolName));
...
function comparePolicyOrder(a, b) {
	const byPosition = a.position.localeCompare(b.position);
	if (byPosition !== 0) return byPosition;
	return a.id.localeCompare(b.id);
}
function policyAppliesToActor(policy, workspaceId, actor) {
	if (policy.workspaceId !== workspaceId) return false;
	if (policy.scope === "workspace") return true;
	if (policy.scope !== "agent") return false;
	const policyAgentId = nonEmpty(policy.agentId);
	if (policyAgentId) return policyAgentId === actor.agentId;
	const policyAgentFamily = nonEmpty(policy.agentFamily);
	if (policyAgentFamily) return policyAgentFamily === actor.agentFamily;
	return false;
}
function policyOperationApplies(policy, operation) {
	const policyOperation = policy.operation ?? "*";
	return policyOperation === "*" || policyOperation === operation;
}
...
function parseTarget(value) {
	...
	if (!parts.every((part) => POLICY_TOKEN_PATTERN.test(part))) return null;
	return parts;
}
function parsePattern(value) {
	...
	for (const part of parts) {
		if (part === "*" || POLICY_TOKEN_PATTERN.test(part)) continue;
		return null;
	}
	return parts;
}
function matchesPattern(pattern, target) {
	...
	return pattern.every((part, index) => part === "*" || part === target[index]);
}
//#endregion
export { PolicyActorKind, PolicyActorSchema, PolicyDecisionAction, PolicyOperation, PolicyRuleRecord, PolicyScope, SourcePolicy, compatibilityPolicyModifiers, compileSourcePolicy, normalizePolicyIdentifier, policyTarget, resolvePolicyRule, resolveToolExecutionPolicy, validatePolicyPattern };
