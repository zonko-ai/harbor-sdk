...
const ToolBindingKind$1 = Schema.Literals(TOOL_BINDING_KINDS$1);
...
//#endregion
//#region ../registry-catalog/data/v1/availability.json
var availability_default = {
	manual_oauth_setup_slugs: [
		"airtable-mcp",
		"clickup-mcp",
		"dropbox-mcp",
		"intercom-mcp",
	...
	global_client_enabled_slugs: ["github-mcp"],
	known_broken_slugs: [
		"echo-cli",
		"atlassian-mcp",
		"bitly-mcp",
	...
	superseded_by_kind: {
		...
		"github-graphql": "github-rest-api",
		"vercel-cli": "vercel-api",
		"vercel-mcp": "vercel-api",
		"wrangler-cli": "cloudflare-api",
		"cloudflare-mcp": "cloudflare-api",
		"glab-cli": "gitlab-rest-api",
		"linear-mcp": "linear-graphql",
		"digitalocean-mcp": "digitalocean-api",
		"asana-mcp": "asana-api",
		"axiom-mcp": "axiom-api",
		"stripe-mcp": "stripe-api",
		"figma-mcp": "figma-api",
		"openai-mcp": "openai-api"
	},
	install_verification_pending_slugs: [
		"brightdata-mcp",
		"fal-mcp",
		"freshdesk-mcp",
		"scraperapi-mcp",
...
const REGISTRY_CATALOG_SLUGS = entries;
...
const REGISTRY_CATALOG_AVAILABILITY = availability_default;
//#endregion
//#region ../catalog/src/index.ts
function getCatalogAvailabilityRules() {
	return REGISTRY_CATALOG_AVAILABILITY;
}
Object.fromEntries(Object.entries(REGISTRY_CATALOG_LOCAL_ICONS).map(([slug, icon]) => [slug, icon.kind === "single" ? icon.path : icon.light]));
REGISTRY_CATALOG_ENTRIES.map((entry) => Schema.decodeUnknownSync(PluginRegistryEntry)(entry));
//#endregion
//#region ../policy/src/source.ts
...
function stoppedExposure(code, reason = comingSoonReason[code]) {
	return {
		status: "coming_soon",
		selectable: false,
...
function withDiagnostic(policy, modifier, message) {
	return {
		...policy,
...
function entryConfig$1(entry) {
	return entry.config && typeof entry.config === "object" ? entry.config : {};
}
function isSseOnly(entry) {
	return entry.kind === "mcp" && entryConfig$1(entry).mcp_transport === "sse";
}
function inferTransport(entry) {
	if (entry.kind === "cli") return "cli";
	if (entry.kind === "composio") return "composio";
	if (entry.kind === "api") return entryConfig$1(entry).api_protocol === "graphql" ? "api_graphql" : "api_http";
	return isSseOnly(entry) ? "mcp_sse" : "mcp_http";
}
function inferToolBindingKinds(entry) {
	const kinds = /* @__PURE__ */ new Set();
	for (const tool of entry.manifest?.tools ?? []) {
		const kind = tool.binding.kind;
		if (TOOL_BINDING_KINDS$1.includes(kind)) kinds.add(kind);
	}
	return [...kinds];
}
function inferCredentialSlots(entry) {
...
function inferBaseAuthKind(entry) {
	return entry.auth.method === "none" ? "none" : "static_secret";
}
function inferBaseInstallFlow(entry) {
	if (entry.auth.method !== "none") return "manual_credentials";
	if (entry.kind === "mcp") return "discover";
	return "direct";
}
function inferCapabilities(entry) {
	const capabilities = new Set([entry.category]);
	for (const tool of entry.manifest?.tools ?? []) for (const tag of tool.tags ?? []) capabilities.add(tag);
	return [...capabilities];
}
function baseSourcePolicy(entry) {
	return {
		identity: {
			slug: entry.slug,
			kind: entry.kind,
		...
		agent: { capabilities: inferCapabilities(entry) },
		adapters: {},
		diagnostics: [{
			phase: "catalog",
			modifier_id: "base",
			message: "Compiled base source policy from catalog entry."
		}]
	};
}
function hasSlug(values, slug) {
	return values.includes(slug);
}
function isStopped(policy) {
	return policy.exposure.status !== "active";
}
const compatibilityPolicyModifiers = [
	{
		...
		apply(policy, input) {
			...
			return withDiagnostic({
				...policy,
				exposure: stoppedExposure("known_broken")
			}, this, "Stopped by catalog curation policy.");
		}
	},
	{
		...
		apply(policy, input) {
			...
			return withDiagnostic({
				...policy,
				exposure: {
					...stoppedExposure("superseded_by_kind", comingSoonReason.superseded_by_kind + " Use `" + supersededBy + "` instead."),
	...
	},
	{
		...
		apply(policy, input) {
			...
			return withDiagnostic({
				...policy,
				exposure: stoppedExposure("sse_only")
			}, this, "SSE-only MCP transport is not active.");
		}
	},
	{
		...
		apply(policy, input) {
			...
			const configured = hasSlug(input.rules.global_client_enabled_slugs, input.entry.slug) && (configuredByEntry || configuredByContext);
			return withDiagnostic({
				...policy,
	...
	},
	{
		...
		apply(policy, input) {
			...
			return withDiagnostic({
				...
				setup: {
					...policy.setup,
					auth_kind: "manual_client_oauth",
					install_flow: "manual_credentials"
				}
	...
	},
	{
		...
		apply(policy, input) {
			...
			return withDiagnostic({
				...policy,
				exposure: stoppedExposure("install_verification_pending")
			}, this, "Install verification is pending.");
		}
	}
];
function compileSourcePolicy(entry, context = {}, modifiers = compatibilityPolicyModifiers) {
	const input = {
		entry,
		context,
		rules: context.catalog?.availabilityRules ?? getCatalogAvailabilityRules()
	};
	return [...modifiers].sort((a, b) => a.priority - b.priority).reduce((policy, modifier) => modifier.apply(policy, input), baseSourcePolicy(entry));
}
//#endregion
//#region ../policy/src/index.ts
const PolicyDecisionAction = Schema.Literals([
	"allow",
	"deny",
	"require_approval",
	"audit_only"
]);
const PolicyOperation = Schema.Literals([
	"*",
	"tool.invoke",
	"exec.preflight",
	"source.install",
	...
	"credential.upsert",
	"credential.delete",
	"workspace.oauth_client.set",
	"workspace.oauth_client.delete",
	"deploy.register",
	"deploy.activate",
	"deploy.disable",
...
const PolicyScope = Schema.Literals(["workspace", "agent"]);
const PolicyActorKind = Schema.Literals([
	"user",
	"agent",
	"sdk",
	"local_mcp",
...
Schema.Struct({
	...
	agentFamily: Schema.optional(Schema.NullOr(Schema.String))
});
Schema.Struct({
	id: Schema.String,
	workspace_id: Schema.String,
	...
	agent_family: Schema.optional(Schema.NullOr(Schema.String)),
	created_by: Schema.optional(Schema.NullOr(Schema.String)),
...
//#endregion
//#region ../provider-overrides/src/index.ts
Schema.Struct({
	id: AdapterId,
	catalog_slugs: Schema.Array(CatalogSlug),
	config_key: Schema.optional(Schema.String),
	auth_kind: Schema.optional(AuthKind),
	install_flow: Schema.optional(InstallFlow),
	adapters: Schema.Record(Schema.String, AdapterId),
	runtime: Schema.optional(Schema.Struct({
		api_key_env: Schema.optional(Schema.String),
		callback_path: Schema.optional(Schema.String),
		client_id_env: Schema.optional(Schema.String),
		client_secret_env: Schema.optional(Schema.String),
		pending_code_verifier: Schema.optional(Schema.String),
		connected_account_session_key: Schema.optional(Schema.String)
	}))
});
const providerOverrideDescriptors = [{
	id: "provider.composio.managed_account",
	catalog_slugs: [],
	config_key: "composio_auth_config_id",
	auth_kind: "managed_account",
	install_flow: "managed_provider",
	adapters: {
		auth: "composio-managed",
		request: "composio-managed-mcp",
		discovery: "composio-managed-mcp"
	},
	runtime: {
		api_key_env: "COMPOSIO_API_KEY",
		callback_path: "/auth/composio/callback",
		pending_code_verifier: "composio",
		connected_account_session_key: "composio_connected_account_id"
	}
}];
function entryConfig(entry) {
	return entry.config && typeof entry.config === "object" ? entry.config : {};
}
function matchesDescriptor(descriptor, entry) {
	if (descriptor.catalog_slugs.includes(entry.slug)) return true;
	if (descriptor.config_key && Object.prototype.hasOwnProperty.call(entryConfig(entry), descriptor.config_key)) return true;
	return false;
}
function readComposioStaticAuthConfig(config) {
	const raw = config.composio_static_auth;
	if (!raw || typeof raw !== "object") return void 0;
	const candidate = raw;
	const authScheme = candidate.auth_scheme;
	const credentialMap = candidate.credential_map;
	if (authScheme !== "API_KEY" && authScheme !== "BEARER_TOKEN" && authScheme !== "BASIC") return;
	if (!credentialMap || typeof credentialMap !== "object" || Array.isArray(credentialMap)) return;
	const normalizedMap = {};
	for (const [fieldName, envKey] of Object.entries(credentialMap)) {
		if (!fieldName || typeof envKey !== "string" || envKey.length === 0) return void 0;
		normalizedMap[fieldName] = envKey;
	}
	return {
		auth_scheme: authScheme,
		credential_map: normalizedMap,
		...typeof candidate.validate_credentials === "boolean" ? { validate_credentials: candidate.validate_credentials } : {}
	};
}
function managedAccountSourceConfig(config) {
	const descriptor = providerOverrideDescriptors.find((candidate) => candidate.auth_kind === "managed_account" && candidate.config_key && typeof config[candidate.config_key] === "string");
	if (!descriptor?.config_key) return void 0;
	const authConfigId = config[descriptor.config_key];
	if (typeof authConfigId !== "string" || authConfigId.length === 0) return void 0;
	const staticAuth = readComposioStaticAuthConfig(config);
	return {
...
function managedAccountSourceConfigPatch(config) {
	const descriptor = providerOverrideDescriptors.find((candidate) => candidate.auth_kind === "managed_account" && candidate.config_key && typeof config[candidate.config_key] === "string");
	if (!descriptor?.config_key) return {};
	const value = config[descriptor.config_key];
	if (typeof value !== "string" || value.length === 0) return {};
	const staticAuth = readComposioStaticAuthConfig(config);
	return {
		[descriptor.config_key]: value,
		...staticAuth ? { composio_static_auth: staticAuth } : {}
	};
}
function managedAccountDescriptor() {
	return providerOverrideDescriptors.find((candidate) => candidate.auth_kind === "managed_account" && typeof candidate.runtime?.callback_path === "string");
}
function managedAccountConnectedAccountId(session) {
	if (typeof session.connected_account_id === "string") return session.connected_account_id;
	for (const descriptor of providerOverrideDescriptors) {
		const key = descriptor.runtime?.connected_account_session_key;
		if (key && typeof session[key] === "string") return session[key];
	}
}
function readManagedAccountSourceHandle(source) {
	if (typeof source.connected_account_id === "string") return source.connected_account_id;
	for (const descriptor of providerOverrideDescriptors) {
		const key = descriptor.runtime?.connected_account_session_key;
...
function canonicalizeOAuthAuthorizationEndpoint(endpoint) {
	return endpoint;
}
function canonicalizeMcpTransportEndpoint(endpoint) {
	return endpoint;
}
function applyOAuthAuthorizationScopeParam(authUrl, scope) {
	authUrl.searchParams.set("scope", scope);
}
function normalizeProviderOAuthTokens(parsed) {}
function providerOverridePolicyModifier(descriptor) {
	return {
		id: descriptor.id,
		phase: "curation",
		priority: 1e3,
		apply(policy, input) {
			if (!matchesDescriptor(descriptor, input.entry)) return policy;
			return {
				...policy,
				setup: {
					...policy.setup,
					...descriptor.auth_kind ? { auth_kind: descriptor.auth_kind } : {},
					...descriptor.install_flow ? { install_flow: descriptor.install_flow } : {}
				},
				adapters: {
					...policy.adapters,
					...descriptor.adapters
				...
				diagnostics: [...policy.diagnostics, {
					phase: "curation",
					modifier_id: descriptor.id,
					message: "Applied provider override descriptor."
				}]
			};
		}
	};
}
const providerOverridePolicyModifiers = providerOverrideDescriptors.map(providerOverridePolicyModifier);
function compileProviderAwareSourcePolicy(entry, context = {}) {
	return compileSourcePolicy(entry, context, [...compatibilityPolicyModifiers, ...providerOverridePolicyModifiers]);
}
function sourcePolicyAllowsGlobalClientActivation(entry) {
	if (getCatalogAvailabilityRules().global_client_enabled_slugs.includes(entry.slug)) return true;
	return providerOverrideDescriptors.some((descriptor) => descriptor.auth_kind === "global_confidential_oauth" && descriptor.install_flow === "discover_then_auth" && matchesDescriptor(descriptor, entry));
}
//#endregion
export { applyOAuthAuthorizationScopeParam, canonicalizeMcpTransportEndpoint, canonicalizeOAuthAuthorizationEndpoint, compileProviderAwareSourcePolicy, managedAccountConnectedAccountId, managedAccountDescriptor, managedAccountSourceConfig, managedAccountSourceConfigPatch, normalizeProviderOAuthTokens, readManagedAccountSourceHandle, sourcePolicyAllowsGlobalClientActivation };

//# sourceMappingURL=provider-overrides.mjs.map