...
const joinUrl$1 = (baseUrl, path) => baseUrl.replace(/\/+$/, "") + path;
...
function createHarborGeneratedClient(options) {
	...
	const request = async (method, path, body, init) => {
		...
		const response = await fetchImpl(joinUrl$1(options.baseUrl, path), requestInit);
...
//#endregion
//#region src/auth.ts
var HarborAuthError = class extends Error {
	status;
	payload;
	constructor(status, payload) {
		const message = payload && typeof payload === "object" && "error" in payload && typeof payload.error === "string" ? payload.error : status === 0 ? "Harbor auth request failed" : "Harbor auth request failed with HTTP " + status;
		super(message);
...
const unwrapEnvelope = (payload) => {
	if (isRecord(payload) && payload.success === true && "data" in payload) return payload.data;
	return payload;
};
const postAuth = async (options, path, body, apiKey) => {
	const fetchImpl = options.fetch ?? globalThis.fetch;
	if (!fetchImpl) throw new Error("No fetch implementation available for Harbor auth");
	const headers = {
		accept: "application/json",
	...
	const payload = await response.json().catch(() => void 0);
	if (!response.ok || isRecord(payload) && payload.success === false) throw new HarborAuthError(response.status, payload);
	return unwrapEnvelope(payload);
};
const startHarborDeviceLogin = async (options) => {
	const payload = await postAuth(options, HARBOR_AUTH_ROUTES.device, {});
	if (!isRecord(payload) || typeof payload.device_code !== "string" || typeof payload.user_code !== "string" || typeof payload.verification_uri_complete !== "string" || typeof payload.interval !== "number" || typeof payload.expires_in !== "number") throw new HarborAuthError(0, { error: "Invalid device login start response" });
	return {
...
const exchangeWorkspaceApiKey = async (options, switchApiKey, workspaceId) => {
	const payload = await postAuth(options, HARBOR_AUTH_ROUTES.workspaceKey, { workspace_id: workspaceId }, switchApiKey);
	if (!isRecord(payload) || typeof payload.api_key !== "string") throw new HarborAuthError(0, { error: "Invalid workspace API key response" });
	return payload.api_key;
};
const pollHarborDeviceLogin = async (options) => {
	const payload = await postAuth(options, HARBOR_AUTH_ROUTES.poll, { device_code: options.deviceCode });
	if (!isRecord(payload) || typeof payload.status !== "string") throw new HarborAuthError(0, { error: "Invalid device login poll response" });
	if (payload.status === "pending") return { status: "pending" };
	if (payload.status === "expired") return { status: "expired" };
	if (payload.status !== "authorized" || typeof payload.api_key !== "string" || typeof payload.workspace_id !== "string") throw new HarborAuthError(0, { error: "Invalid authorized device login response" });
	const user = isRecord(payload.user) ? payload.user : {};
	const userName = user.name;
	const workspaceId = options.workspaceId ?? payload.workspace_id;
	const switchApiKey = payload.api_key;
	const apiKey = await exchangeWorkspaceApiKey(options, switchApiKey, workspaceId);
	return {
		status: "authorized",
		apiKey,
		workspaceId,
...
//#endregion
export { HarborApiError, HarborAuthError, HarborClientConfigurationError, HarborWorkspaceResolutionError, createHarborClient, createHarborGeneratedClient, createHarborOAuthAuthorizeUrl, createHarborPromiseClient, pollHarborDeviceLogin, startHarborDeviceLogin };
