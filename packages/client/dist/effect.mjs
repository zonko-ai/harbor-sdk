...
const isPromiseLike = (value) => !!value && (typeof value === "object" || typeof value === "function") && "then" in value && typeof value.then === "function";
const toEffectClient = (value) => {
	if (typeof value === "function") return ((...args) => {
		try {
			const result = value(...args);
			if (isPromiseLike(result)) return Effect.tryPromise({
				try: async () => await result,
				catch: (error) => error
			});
			return toEffectClient(result);
		} catch (error) {
	...
	const output = {};
	for (const key of Reflect.ownKeys(value)) output[key] = toEffectClient(value[key]);
	return output;
};
const createHarborEffectClient = (options) => toEffectClient(createHarborPromiseClient(options));
const HarborClientTag = Context.Service("@hrbr/client/effect/HarborClient");
const HarborClient = Object.assign(HarborClientTag, { layer: (options) => Layer.succeed(HarborClientTag, createHarborEffectClient(options)) });
//#endregion
export { HarborClient, HarborClientConfigurationError, HarborWorkspaceResolutionError, createHarborEffectClient, createHarborOAuthAuthorizeUrl };
